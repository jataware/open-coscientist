...
def get_generation_prompt(
    ...
    variables = {
        ...
        "preferences": preferences or "Novel, testable, scientifically sound, specific, and diverse hypotheses",
        "attributes": ", ".join(attributes) if attributes and isinstance(attributes, list) else (attributes or "N/A"),
        "user_hypotheses": "\n".join(f"- {hyp}" for hyp in user_hypotheses) if user_hypotheses and isinstance(user_hypotheses, list) else (user_hypotheses or "N/A"),
        "instructions": instructions or f"Generate {hypotheses_count} diverse and novel hypotheses."
    }
    ...
    if supervisor_guidance and isinstance(supervisor_guidance, dict):
        ...
        if key_areas:
            if not has_content:
                ...
                guidance_sections.append("The Supervisor Agent has analyzed the research goal and provided the following guidance to inform your hypothesis generation:\n")
                has_content = True
        ...
        if generation_phase:
            ...
            if generation_phase.get("diversity_targets"):
                guidance_sections.append(f"**Diversity Targets:** {generation_phase['diversity_targets']}\n")
            if generation_phase.get("quantity_target"):
                guidance_sections.append(f"**Quantity Target:** {generation_phase['quantity_target']}\n")
...
def get_evolution_prompt(
    ...
    review_feedback: str,
    meta_review_insights: str
) -> Tuple[str, Optional[Dict[str, Any]]]:
    """Get the hypothesis evolution prompt and schema."""
    return load_prompt_with_schema("evolution", {
...
def get_ranking_prompt(
    ...
    variables["hypothesis_a_reflection_notes"] = reflection_notes_a or "No reflection notes available."
    variables["hypothesis_b_reflection_notes"] = reflection_notes_b or "No reflection notes available."
...
def get_meta_review_prompt(
    ...
    # Add supervisor guidance if available
    variables["supervisor_guidance"] = _format_supervisor_guidance_for_meta_review(supervisor_guidance)
...
def get_proximity_prompt(
    ...
    import json
    variables = {
        "hypotheses": json.dumps([h["text"] if isinstance(h, dict) else h for h in hypotheses], indent=2)
    }

    # Add supervisor guidance if available
    variables["supervisor_guidance"] = _format_supervisor_guidance_for_proximity(supervisor_guidance)
...
def get_supervisor_prompt(
    ...
    if pubmed_available or mcp_available:
        lit_review_description = "literature review will search pubmed for relevant papers and analyze them"
    else:
        lit_review_description = "literature review is not available (no pubmed access)"

    return load_prompt_with_schema("supervisor", {
        "research_goal": research_goal,
        ...
        "user_literature": "\n".join(f"- {lit}" for lit in user_literature) if user_literature else "None provided",
...
def _format_meta_review_context(meta_review: Dict[str, Any] | None) -> str:
    ...
    sections.append("The following insights were synthesized from previous reviews of all hypotheses:\n\n")
...
def _format_review_context(review_a: Dict[str, Any] | None, review_b: Dict[str, Any] | None) -> str:
    ...
    sections.append("Consider these scores, but make your judgment based on comprehensive comparison, not just scores.\n")
...
def _format_supervisor_guidance_for_meta_review(supervisor_guidance: Dict[str, Any] | None) -> str:
    ...
    sections.append("Use this guidance to ensure your meta-review synthesis aligns with the research plan and evolution strategy.\n")
...
def _format_evolution_details_context(evolution_details: List[Dict[str, Any]] | None) -> str:
    ...
    if not evolution_details or not isinstance(evolution_details, list) or len(evolution_details) == 0:
        return ""
    
    sections = []
    sections.append("## Previous Evolution History\n")
    sections.append("The following hypotheses were evolved in previous iterations. Consider what changes were made and their effectiveness:\n\n")
    ...
    sections.append("Use this history to inform your recommendations and avoid repeating ineffective changes.\n")
...
def get_reflection_prompt(
    ...
    return load_prompt_with_schema("reflection_observations", {
...
def get_literature_review_query_generation_pubmed_prompt(
    ...
    return load_prompt("literature_review_query_generation_pubmed", {
        "research_goal": research_goal,
        "preferences": preferences if preferences else "None provided",
        "attributes": ", ".join(attributes) if attributes else "None provided",
        "user_literature": "\n".join(f"- {lit}" for lit in user_literature) if user_literature else "None provided",
        "user_hypotheses": "\n".join(f"- {hyp}" for hyp in user_hypotheses) if user_hypotheses else "None provided"
    })
...
def get_literature_review_paper_analysis_prompt(
    ...
    return load_prompt("literature_review_paper_analysis", {
        "research_goal": research_goal,
        "title": title,
        "authors": authors_str,
        "year": year_str,
...
"""
    ...
    return load_prompt("literature_review_synthesis", {
        "research_goal": research_goal,
        "paper_analyses": "\n\n".join(analyses_text)
    })
...
def get_hypothesis_novelty_analysis_prompt(
    ...
    return load_prompt("hypothesis_novelty_analysis", {
        "hypothesis_text": hypothesis_text,
        "title": title,
        "authors": authors_str,
        "year": year_str,
...
"""
    ...
    return load_prompt("hypothesis_validation_synthesis", {
        "research_goal": research_goal,
...
def get_debate_generation_prompt(
    ...
    variables = {
        ...
        "preferences": preferences or "Novel, testable, scientifically sound, specific, and diverse hypotheses",
        "attributes": ", ".join(attributes) if attributes and isinstance(attributes, list) else (attributes or "testable and falsifiable"),
    }
...
def get_draft_prompt_with_tools(
    ...
    variables = {
        ...
        "articles_with_reasoning": articles_with_reasoning or "no literature review summary available - examine papers below directly.",
        "articles_metadata": format_articles_metadata(articles or []),
        "max_iterations": max_iterations,
        "instructions": instructions or "Focus on creative ideation - draft diverse hypotheses based on literature gaps."
    }
